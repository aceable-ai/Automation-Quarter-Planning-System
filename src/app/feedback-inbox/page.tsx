          ...
          {filtered.map(item => {
            ...
            return (
              <div key={item.id} style={{
                ...
                borderLeft: item.status === 'pending' ? '4px solid #f59e0b' : item.status === 'approved' ? '4px solid #16a34a' : '4px solid #d1d5db',
              }}>
                <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12 }}>
                  ...
                  {item.status === 'pending' && (
                    <div style={{ display: 'flex', gap: 6, flexShrink: 0 }}>
                      <button onClick={() => { setApproving(item.id); setApproveProject(item.project_id ?? ''); }}
