...
export default function DiagramPage() {
  ...
  useEffect(() => {
    void Promise.all([
      fetch(`/api/diagrams/${id}`).then(r => r.json() as Promise<{ nodes: Node[]; edges: Edge[] }>),
      fetch(`/api/master-projects/${id}`).then(r => r.json() as Promise<{ name: string }>),
    ]).then(([d, p]: [{ nodes: Node[]; edges: Edge[] }, { name: string }]) => {
      setNodes(d.nodes);
      setEdges(d.edges);
    ...
    <div style={{ height: '100vh', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ padding: '12px 20px', background: '#fff', borderBottom: '1px solid #e5e7eb', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div style={{ fontSize: 13, color: '#9ca3af' }}>
          ...
          <Link href={`/master-plans/${id}`} style={{ color: '#6366f1', textDecoration: 'none' }}>{projectName}</Link>
          <span style={{ margin: '0 6px' }}>/</span>
          <span style={{ color: '#374151', fontWeight: 600 }}>Architecture Diagram</span>
        </div>
        <div style={{ fontSize: 11, color: '#9ca3af' }}>Auto-saves &middot; Double-click to edit labels &middot; Drag handles to connect</div>
      </div>
          ...
          <Panel position="top-left">
            <div style={{
              ...
              <div style={{ display: 'flex', gap: 4 }}>
                {SHAPE_OPTIONS.map(s => (
                  <button key={s.type} onClick={() => setSelectedShape(s.type)}
                    title={s.label}
                    style={{
              ...
              <button onClick={addNode}
                ...
                + Add {SHAPE_OPTIONS.find(s => s.type === selectedShape)?.label}
              </button>
